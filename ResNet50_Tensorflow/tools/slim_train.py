...
from ResNet50_Tensorflow.nets.ResNet50_slim import ResNet50
...
# pretrain model
pretrain_model_path = '/home/alex/Documents/pretrain_model/resnet/resnet_50/resnet_v2_50.ckpt'
...
if __name__ == "__main__":
    ...
    with tf.Session(config=config) as sess:
        ...
        logit_op = resnet_50.logits.op.name
        ...
        logit_biases = graph.get_tensor_by_name(name='resnet_v2_50/logits/biases:0')
        ...
        if FLAGS.is_pretrain:
            ...
            resnet_50.load_weights(sess, model_path=FLAGS.pretrain_model_path)
        ...
        try:
            if not coord.should_stop():
                ...
                for step in range(max_step):
                    ...
                    feed_dict = resnet_50.fill_feed_dict(image_feed=train_image, label_feed=train_label,
                                                         is_training=True)

                    _, train_loss, train_accuracy, summary = sess.run(fetches=[resnet_50.train, resnet_50.loss,
                                                                               resnet_50.accuracy, summary_op],
                                                                      feed_dict=feed_dict)
                    ...
                    if (step + 1) % FLAGS.save_step_period == 0:
                        saver.save(sess, save_path=os.path.join(FLAGS.model_dir, 'model.ckpt'),
                                   global_step=resnet_50.global_step)
                    # ++++++++++++++++++++++++++++++++validation part++++++++++++++++++++++++++++++++++++++++++++
                    ...
                    if (step + 1) % step_per_epoch == 0:
                        ...
                        for _ in range(val_max_steps):
                            ...
                            feed_dict = resnet_50.fill_feed_dict(image_feed=val_images, label_feed=val_labels,
                                                                 is_training=False)

                            val_loss, val_acc = sess.run([resnet_50.loss, resnet_50.accuracy],
                                                         feed_dict=feed_dict)
                ...
                input_op = resnet_50.raw_input_data.op.name
                logit_op = resnet_50.logits.op.name
